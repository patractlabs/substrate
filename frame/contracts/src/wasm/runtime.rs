...
use jupiter_io::zk_snarks::{
    altbn_128_add, altbn_128_mul, altbn_128_pairing,
    bls12_381_add, bls12_381_mul, bls12_381_pairing,
};
...
pub enum RuntimeToken {
	...
	HashBlake128(u32),
    /// Weight of calling `seal_curve_altbn_128_add` for the given input size.
    CurveAltBn128Add(u32),
    /// Weight of calling `seal_curve_altbn_128_mul` for the given input size.
    CurveAltBn128Mul(u32),
    /// Weight of calling `seal_curve_altbn_128_pairing` for the given input size.
    CurveAltBn128Pairing(u32),
    /// Weight of calling `seal_curve_bls12_381_add` for the given input size.
    CurveBls12381Add(u32),
    /// Weight of calling `seal_curve_bls12_381_mul` for the given input size.
    CurveBls12381Mul(u32),
    /// Weight of calling `seal_curve_bls12_381_pairing` for the given input size.
    CurveBls12381Pairing(u32),
}
	...
	T::AccountId: UncheckedFrom<T::Hash>, T::AccountId: AsRef<[u8]>
{
	...
	fn calculate_amount(&self, s: &Self::Metadata) -> Gas {
		...
		match *self {
            ...
            CurveAltBn128Add(len) => s.curve_altbn_128_add
                .saturating_add(s.curve_altbn_128_add_per_byte.saturating_mul(len.into())),
            CurveAltBn128Mul(len) => s.curve_altbn_128_mul
                .saturating_add(s.curve_altbn_128_mul_per_byte.saturating_mul(len.into())),
            CurveAltBn128Pairing(len) => s.curve_altbn_128_mul
                .saturating_add(s.curve_altbn_128_pairing_per_byte.saturating_mul(len.into())),
            CurveBls12381Add(len) => s.curve_bls12_381_add
                .saturating_add(s.curve_bls12_381_add_per_byte.saturating_mul(len.into())),
            CurveBls12381Mul(len) => s.curve_bls12_381_mul
                .saturating_add(s.curve_bls12_381_mul_per_byte.saturating_mul(len.into())),
            CurveBls12381Pairing(len) => s.curve_bls12_381_mul
                .saturating_add(s.curve_bls12_381_pairing_per_byte.saturating_mul(len.into())),
		}
	}
}
		...
		UncheckedFrom<<E::T as frame_system::Config>::Hash> + AsRef<[u8]>
{
    ...
    ///
    /// The `input` and `output` buffers may overlap.
    fn compute_curve_on_intermediate_buffer<F, R>(
        &mut self,
        inflect_fn: F,
        input_ptr: u32,
        input_len: u32,
...
define_env!(Env, <E: Ext>,
    ...
    // - `input_len`: the length of the input data in bytes.
    // - `output_ptr`: the pointer into the linear memory where the g2 data is placed.
    // - `output_len`: the length of the g2 data in bytes.
    // - `result_ptr`: the pointer into the linear memory where the result
    //                 data is placed. The function will write the result
    //                 directly into this buffer.
    seal_curve_altbn_128_add(ctx, input_ptr: u32, input_len: u32, output_ptr: u32) => {
        ctx.charge_gas(RuntimeToken::CurveAltBn128Add(input_len))?;
        ctx.compute_curve_on_intermediate_buffer(
            altbn_128_add, input_ptr, input_len, output_ptr,
        )
    },
    ...
    // - `input_ptr`: the pointer into the linear memory where the input data is placed.
    // - `input_len`: the length of the input data in bytes.
    // - `output_ptr`: the pointer into the linear memory where the output data is placed.
    // - `output_len`: the length of the output data in bytes.
    // - `result_ptr`: the pointer into the linear memory where the result
    //                 data is placed. The function will write the result
    //                 directly into this buffer.
    seal_curve_altbn_128_mul(ctx, input_ptr: u32, input_len: u32, output_ptr: u32) => {
        ctx.charge_gas(RuntimeToken::CurveAltBn128Mul(input_len))?;
        ctx.compute_curve_on_intermediate_buffer(
            altbn_128_mul, input_ptr, input_len, output_ptr,
        )
    },
    ...
    // # Parameters
    //
    // - `input_ptr`: the pointer into the linear memory where the g1 data is placed.
    // - `input_len`: the length of the g1 data in bytes.
    ...
    //                 data is placed. The function will write the result
    //                 directly into this buffer.
    seal_curve_bls12_381_add(ctx, input_ptr: u32, input_len: u32, output_ptr: u32) => {
        ctx.charge_gas(RuntimeToken::CurveBls12381Add(input_len))?;
        ctx.compute_curve_on_intermediate_buffer(
            bls12_381_add, input_ptr, input_len, output_ptr,
        )
    },
    // Computes the ALTBN128 mul on the given input buffer.
    //
    ...
    //                 data is placed. The function will write the result
    //                 directly into this buffer.
    seal_curve_bls12_381_mul(ctx, input_ptr: u32, input_len: u32, output_ptr: u32) => {
        ctx.charge_gas(RuntimeToken::CurveBls12381Mul(input_len))?;
        ctx.compute_curve_on_intermediate_buffer(
            bls12_381_mul, input_ptr, input_len, output_ptr,
        )
    },
    // Computes the ALTBN128 pairing on the given input buffer.
    //
