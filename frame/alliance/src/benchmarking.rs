...
fn set_blacklist<T: Config<I>, I: 'static>() {
	...
	let mut website_blacklist = (0..T::MaxBlacklistCount::get())
		.map(|i| vec![i as u8])
		.collect::<Vec<_>>();
...
benchmarks_instance_pallet! {
	set_rule {
		...
		let rule = test_cid();
		let call = Call::<T, I>::set_rule(rule.clone());
	...
	approve_candidate {
		...
		let call = Call::<T, I>::approve_candidate(candidate1_lookup);
		let origin = T::SuperMajorityOrigin::successful_origin();
	}: { call.dispatch_bypass_filter(origin)? }
	verify {
		assert!(!Alliance::<T, I>::is_candidate(&candidate1));
	...
	reject_candidate {
		...
		let candidate1_lookup: <T::Lookup as StaticLookup>::Source = T::Lookup::unlookup(candidate1.clone());
		let call = Call::<T, I>::reject_candidate(candidate1_lookup);
		let origin = T::SuperMajorityOrigin::successful_origin();
	}: { call.dispatch_bypass_filter(origin)? }
	verify {
		assert!(!Alliance::<T, I>::is_candidate(&candidate1));
	...
	elevate_ally {
		...
		let call = Call::<T, I>::elevate_ally(ally1_lookup);
		let origin = T::SuperMajorityOrigin::successful_origin();
	}: { call.dispatch_bypass_filter(origin)? }
	verify {
		assert!(!Alliance::<T, I>::is_ally(&ally1));
	...
	kick_member {
		...
		let call = Call::<T, I>::kick_member(fellow2_lookup);
		let origin = T::SuperMajorityOrigin::successful_origin();
	}: { call.dispatch_bypass_filter(origin)? }
	verify {
		assert!(!Alliance::<T, I>::is_member(&fellow2));
	...
	add_blacklist {
		...
		let call = Call::<T, I>::add_blacklist(blacklist.clone());
		let origin = T::SuperMajorityOrigin::successful_origin();
	}: { call.dispatch_bypass_filter(origin)? }
	verify {
	...
	remove_blacklist {
		...
		let call = Call::<T, I>::remove_blacklist(blacklist.clone());
		let origin = T::SuperMajorityOrigin::successful_origin();
	}: { call.dispatch_bypass_filter(origin)? }
	verify {
